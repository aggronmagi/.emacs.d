...
(add-to-list 'org-capture-templates '("wt" "Work Task" entry
	       (file+olp "~/org/work.org" "工作记录" "Task")
	       "*** TODO %?\n%U\n"))
(add-to-list 'org-capture-templates '("wb" "Work Bug Record" entry
               (file+olp "~/org/work.org" "工作记录" "BUG")
               "*** TODO BUG %?\n%U\n"))
(add-to-list 'org-capture-templates '("wr" "Record Something" entry
...
** TODO [C] 多列编辑
   实现:
     1. 抓取已经mark的region. 重复按多次. 就选取多次. 直接列编辑
     2. 抓取已经mark的region, 按键之后. 光标位于region的行尾. 进入列编辑
